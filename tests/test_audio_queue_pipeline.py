...
def test_process_phrase_runs_pipeline_once_and_removes_temporary_wav() -> None:
    ...
    with patch("audio_queue.synthesize_text", side_effect=synthesize) as synth:
    ...
    assert statuses == ["Listening... (STT test, tts=sam)"]
    assert errors == []
...
def test_run_processes_each_phrase_only_once() -> None:
    ...
    phrase_stream.iter_phrases.return_value = iter([object()])
